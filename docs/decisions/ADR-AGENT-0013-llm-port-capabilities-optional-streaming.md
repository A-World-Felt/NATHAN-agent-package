...
PR2 freezes `LLMProvider`, the port **implemented by the consumer's own providers**. A published interface is a contract: **adding a required method later breaks every implementer, consumers included.** So the method surface must be right *now*, while there are only two implementations to migrate (Ollama and the fake).
